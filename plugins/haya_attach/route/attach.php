...
// 定义插件根目录
define('HAYA_FILE_ROOT', APP_PATH.'plugin/haya_attach/');
...
$menuTab = array(
	'setting' => array(
		'url' => url('attach-setting'),
		'text' => '插件设置',
	),
	'attachs' => array(
		'url' => url('attach-attachs'),
		'text' => '附件管理',
	),
);
...
if (!in_array($action, $actions)) {
	message(1, jump('访问错误', url('attach')));
} 

// 判断文件
$hayaFile = HAYA_FILE_INC.$action.'.php';
if (!file_exists($hayaFile)) {
	message(1, jump($action . '动作不存在', url('attach')));
}
