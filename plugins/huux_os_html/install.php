...
if(!$kv) {
	$kv = array('huux_os_html' =>'{"id":"huux_os_html","name":"大白自定义","type":"插件","set":1}' );
	kv_set('dabai_plugin', $kv);
}else{
	if (!in_array("huux_os_html", $kv)){
		$kv = $kv+array('huux_os_html' =>'{"id":"huux_os_html","name":"大白自定义","type":"插件","set":1}' );
	}
	kv_set('dabai_plugin', $kv);
}
