...
if(empty($action)){
	...
	if($method == 'GET') {
	    ...
	    //2017-08-8 会员注册开关
		$input['user_create_io'] = form_radio_yes_no('user_create_io', (empty($setting['user_create_io'])? 0:$setting['user_create_io']));//会员注册开关
		 // 附件列表
	     $page = 1; //起始页
	     $ajax_list ='';//用于列表回调
	     $num = db_count('attach'); 
	    ...
	    $setting['user_create_io'] = param('user_create_io', 0);//会员注册开关
