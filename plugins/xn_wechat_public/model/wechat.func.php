...
//                    exit;
    ...
    function wechat_create_user ( $data , $openid ) {
        ...
        if( $r ) {
            ...
            $username = xn_substr( $username . '_' . sprintf('%.0f', floatval($t1) * 1000) , 0 , 31 );
            $r = user_read_by_username( $username );
            $r AND message( -1 , '用户名被占用。' );
        }
        // 自动产生一个 Email
        $email = 'qq_'.sprintf('%.0f', (floatval($t1) + floatval($t2))*1000).mt_rand(100,999) .'@qq.com';
        ...
        if( $data['headimgurl'] ) {
            ...
            $data = http_curl_get( $data['headimgurl'] );
            if($data){
